...
import com.example.cofc.databinding.CoursesOfflineBinding;
...
import com.example.cofc.viewmodel.V_OfflineCourses;
...
public class OfflineCourses extends AppCompatActivity {
    ...
    V_OfflineCourses mLive;
    //List<Data_of_course> courses;
    //List<Data_of_news> news;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = DataBindingUtil.setContentView(this , R.layout.courses_offline);
        sp = this.getSharedPreferences(ARG_PARAM1 , Context.MODE_PRIVATE);
        setSupportActionBar(binding.offlineToolbar);

        pager_adapter = new Pager_adapter(getSupportFragmentManager() , 0);
        mLive = new ViewModelProvider(this).get(V_OfflineCourses.class);
        ...
        if (getIntent().getExtras() != null)
        {
            ...
            mLive.get_courses_news(new S_getCourses(sub_type , "offline"));
        }

        mLive.mLive.observe(this, new Observer<R_courses_news>() {
            @Override
            public void onChanged(R_courses_news r_courses_news) {
                if(r_courses_news != null)
                {
                    //total = r_courses_news;
                    //courses = total.getC_list();
                    //news = total.getN_list();
                    pager_adapter.add_fragment(categoryName+" 'Offline", FragmentCourse.newInstance(r_courses_news.getC_list()));
                    pager_adapter.add_fragment("Offline News", FragmentNews.newInstance(r_courses_news.getN_list() , "offline"));
                    binding.offlinePager.setAdapter(pager_adapter);
        ...
        binding.offlineTabs.setupWithViewPager(binding.offlinePager);
    ...
    public void Alert(String w)
    {
        if (sp.getInt(ARG_PARAM1 , -1) != -1)
        {
            ...
            ab.setPositiveButton(R.string.sign_in, (dialog, which) -> {
                Intent i = new Intent(OfflineCourses.this, SignIn.class);
                startActivity(i);
            });
            ab.setNegativeButton(R.string.sign_up, (dialog, which) -> {
                Intent i = new Intent(OfflineCourses.this, SignUp.class);
                startActivity(i);
            });
